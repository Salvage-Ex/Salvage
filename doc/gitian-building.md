...
*Setup instructions for a gitian build of Salvage using a Debian VM or physical system.*
...
Clone the git repositories for salvage and gitian and then checkout the salvage version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the salvage repository
...
To build Salvage (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the salvage repository.
...
```bash
    ...
    From https://github.com/salvage/salvage
...
For example:
```bash
URL=https://github.com/konez2k/salvage.git
...
[salvage/gitian.sigs](https://github.com/salvage/gitian.sigs/) repository, or if that's not possible to create a pull
request.
