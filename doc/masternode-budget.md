...
SecureCloud now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<securecloud_address\> \<monthly_payment_securecloud\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ SCN, _12_ times over the course of a year totaling _24000_ SCN.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<securecloud_address\> \<monthly_payment_securecloud\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` SCN.
 ...
 * vote-many          - Vote on a SecureCloud initiative
 * vote-alias         - Vote on a SecureCloud initiative
 * vote               - Vote on a SecureCloud initiative/budget
